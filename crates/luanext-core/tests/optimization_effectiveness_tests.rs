...
fn devirtualization_final_class() {
    ...
    let o1 = compile(src, OptimizationLevel::Minimal).unwrap();
    let o3 = compile(src, OptimizationLevel::Aggressive).unwrap();
...
fn inlining_o2_vs_o3() {
    ...
    let o2 = compile(src, OptimizationLevel::Moderate).unwrap();
    let o3 = compile(src, OptimizationLevel::Aggressive).unwrap();
...
fn dead_code_after_return() {
    ...
    let _o0 = compile(src, OptimizationLevel::None).unwrap();
    let o1 = compile(src, OptimizationLevel::Minimal).unwrap();
...
fn dead_code_unused_var() {
    ...
    let o0 = compile(src, OptimizationLevel::None).unwrap();
    let o1 = compile(src, OptimizationLevel::Minimal).unwrap();
