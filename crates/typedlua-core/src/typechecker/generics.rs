...
use typedlua_parser::ast::statement::{Parameter, TypeParameter};
...
/// Trait for generic type operations
///
/// This trait provides a unified interface for type parameter instantiation
/// and type argument inference, used by the type checker for generic function
/// and type handling.
pub trait GenericVisitor {
    /// Instantiate a type by substituting type parameters with concrete types
    ///
    /// # Arguments
    /// * `typ` - The type to instantiate (may contain type parameter references)
    /// * `type_params` - The type parameters to substitute
    /// * `type_args` - The concrete types to substitute for the parameters
    ///
    /// # Returns
    /// The instantiated type with all parameters replaced, or an error if
    /// the number of type arguments doesn't match the number of parameters
    fn instantiate_type(
        &self,
        typ: &Type,
    ...
    ///
    /// # Arguments
    /// * `type_params` - The type parameters to infer
    /// * `function_params` - The function's parameters (may reference type parameters)
    /// * `arg_types` - The actual argument types at the call site
    ///
    /// # Returns
    /// A vector of inferred types in the same order as type_params, or an error
    /// if inference fails
    ...
    /// * `type_args` - The type arguments to check
    ///
    /// # Returns
    /// Ok if all constraints are satisfied, or an error describing the violation
...
/// Implementation of the GenericVisitor trait
///
/// This struct provides the concrete implementation of generic type operations.
/// It wraps the free functions in generics.rs to provide a trait-based interface.
pub struct GenericInstantiator;

impl GenericInstantiator {
    /// Create a new GenericInstantiator
    pub fn new() -> Self {
        Self
    }
}
...
impl GenericVisitor for GenericInstantiator {
    fn instantiate_type(
        &self,
        typ: &Type,
        type_params: &[TypeParameter],
        ...
        instantiate_type(typ, type_params, type_args)
    }
    ...
    fn infer_type_arguments(
        ...
        infer_type_arguments(type_params, function_params, arg_types)
    }
    ...
    fn check_type_constraints(
        ...
        check_type_constraints(type_params, type_args)
    }
}
